...
CPU は `ai.ts` の `chooseAction(state, seat)`（戦略であってルールでないため必須APIと分離）。打牌は**向聴数（`shanten.ts`）を最小化**する牌を選び、聴牌ならリーチ宣言/受け入れ最大、同向聴は孤立牌切り。鳴き: 役牌(三元/自風/場風)はポン/カン、タンヤオ志向で聴牌到達ならポン/チー、他は門前維持でパス。自分から暗槓/加槓はしない。
