...
Phase 1-3: 牌・PRNG・和了判定・役・符・点数（実装済み）→ Phase 4a: 局進行・鳴きなし完全ループ（実装済み）→ Phase 4b: **鳴き（チー/ポン/大明槓/暗槓/加槓）実装済み**（`game.ts`。`pendingCalls`/`callResponses` で優先順位 ロン>ポン/カン>チー を解決。嶺上ツモ・カンドラ・槍槓・喰い替え防止・開いた手の点数）→ **途中流局（九種九牌・四風連打・四家立直）・流し満貫・送り槓（待ち不変の暗槓）実装済み**。残り: 四槓散了（未対応）。
