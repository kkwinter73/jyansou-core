...
CPU は `ai.ts` の `chooseAction(state, seat)`（戦略であってルールでないため必須APIと分離）。打牌は**向聴数（`shanten.ts`）を最小化**する牌を選び、聴牌ならリーチ宣言/受け入れ最大、同向聴は孤立牌切り。**押し引き**: 他家リーチ時、`shouldPush`（聴牌は押す／2向聴以上は降りる／1向聴は手の価値=ドラ赤門前・残り巡目・点棒リードで判断）で押す/降りるを決める。降りる場合のベタオリは危険度（現物=0/筋/壁/字牌残数）が最小の牌を選ぶ（複数リーチは最悪ケースを最小化）。鳴き: 役牌(三元/自風/場風)はポン/カン、タンヤオ志向で聴牌到達ならポン/チー、他は門前維持でパス。自分から暗槓/加槓はしない。
